...
### Автозаполнение<br>
Быстро посчитать любое значение по формуле вам поможет функция автозаполнения. Можно написать любую формулу, а затем потянуть чёрный квадрат в правой нижней части ячейки. Второй способ автозаполнения — дважды кликнуть по этому квадрату. Этот способ сработает только в том случае, если соседний столбец слева полностью заполнен.<br>
Если вам нужно «закрепить» какое-то значение (сделать так, чтобы номер ячейки не менялся при автозаполнении), используйте символ «$». Например, формула **=A1 + B$1** при автозаполнении вниз будет преобразовываться так: **=A2 + B$1**, **=A3 + B$1**, **=A4 + B$1**...
